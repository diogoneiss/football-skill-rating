...
impl<'a> LeagueTable<'a> {
    pub fn rank(&self) -> Vec<(&'a str, &TeamStats)> {
        ...
        teams.sort_by(|(a_name, a_stats), (b_name, b_stats)| {
            ...
            b_stats.points.cmp(&a_stats.points)
...
impl<'a> LeagueTable<'a> {
    ...
    pub fn print_final_table(&self) {
        ...
        println!("League Table for the year {}: Division {} of {}", 
                 self.year, self.division, self.campeonato);

        println!("{:<20} {:<7} {:<5} {:<5} {:<7} {:<14} {:<14} {:<8}", 
                 "Team", "Points", "Wins", "Draws", "Losses", "Goals Scored", "Goals Conceded", "Played");

        for (team_name, team_stats) in ranked_teams {
            println!("{:<20} {:<7} {:<5} {:<5} {:<7} {:<14} {:<14} {:<8}", 
                     team_name, 
                     team_stats.points,
                     team_stats.wins,
